...
@XmlRootElement
@NamedQueries({
    @NamedQuery(name = "SieniClase.findByDocente", query = "SELECT c FROM SieniClase c,SieniCurso s,SieniMateriaDocente md where md.idDocente=:idDocente and s.idMateria.idMateria=md.idMateria.idMateria AND s.crEstado='A' and md.idMateria.maEstado='A' and c.idCurso.idCurso=s.idCurso and c.clEstado='A'"),
    ...
    @NamedQuery(name = "SieniClase.findClaseByTipoDocente", query = "SELECT c FROM SieniClase c,SieniCurso s,SieniMateriaDocente md where md.idDocente=:idDocente and s.idMateria.idMateria=md.idMateria.idMateria AND s.crEstado='A' and md.idMateria.maEstado='A' and c.idCurso.idCurso=s.idCurso and c.clEstado='A' and c.clTipo=:tipoClase ORDER BY c.idClase"),
    @NamedQuery(name = "SieniClase.findClaseByTipoAlumno", query = "SELECT distinct c FROM SieniClase c,SieniCursoAlumno ca join ca.idCurso.sieniClaseList cl where cl.idClase=c.idClase and ca.idAlumno=:idAlumno and cl.clEstado not in (:estado) and ca.idCurso.crEstado not in (:estado) and c.clTipo=:tipoClase order by c.idClase"),
//    @NamedQuery(name = "SieniClase.findClaseByTipoAlumno", query = "SELECT s FROM SieniClase s,SieniAlumno al, SieniMatricula m join m.idGrado.sieniCursoList c1 join c1.sieniClaseList cl1 join m.idSeccion.sieniCursoList c2 join c2.sieniClaseList cl2 where cl1.idCurso=cl2.idCurso and cl2.idClase=s.idClase  al.idAlumno=m.idAlumno and s.idCurso m.idGrado.idGrado=s.idCurso.idGrado.idGrado and m.idAlumno=al.idAlumno and s.clEstado not in (:estado) and s.clTipo=:tipoClase and al.idAlumno=:idAlumno  ORDER BY s.idClase"),
...
//    @NamedQuery(name = "SieniClase.findClaseByAlumno", query = "SELECT s FROM SieniClase s,SieniAlumno al join fetch al.sieniMatriculaList m where m.idGrado.idGrado=s.idCurso.idGrado.idGrado and m.idAlumno.idAlumno=al.idAlumno and s.clEstado not in (:estado) and al.idAlumno=:idAlumno"),
    ...
    @NamedQuery(name = "SieniClase.findByHorarioExiste2", query = "SELECT s FROM SieniClase s WHERE s.clHora = :clHora AND s.clTipo='O' AND s.clEstado != 'I'")})
